    ...
    return (
        <main className="p-4">
            <div className='flex flex-col md:flex-row items-start md:space-x-4 space-y-4 md:space-y-0'>
                {character?.image && <Image unoptimized src={character?.image} alt={character?.name?.userPreferred} width={197} height={296} className="w-full md:w-48 h-72 object-cover rounded-lg shadow-lg" priority />}
                ...
                <ScrollArea className='flex flex-nowrap overflow-x-auto whitespace-nowrap py-4'>
                    {voiceActor.map((actor) => (
                        <Card key={actor.id} className='overflow-hidden mx-2 py-2 w-fit text-white border-none inline-block flex-shrink-0'>
                            <Image unoptimized src={actor.image} alt={actor.name.userPreferred} width={100} height={150} className="w-24 h-24 object-cover  rounded-full mx-auto shadow-lg" priority />
