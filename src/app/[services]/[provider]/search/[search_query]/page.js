...
const page = async ({ params }) => {
    ...
    return (
        <Suspense fallback={<h1>Loading search results...</h1>}>
            ...
            <ContentList params={params} headerText={`Search Results for ${decodeURIComponent(params.search_query)}`} data={searchResults} services={services} provider={provider} otherParams="trending" displayStyle="grid" />
        </Suspense>
    )
}
